## perMANOVA and NMDS code for 18S dataset analysis generated and presented in  the manuscript entitled 
...
for(mode in trophic_list){
  ...
  ggsave(glue('./18s_R_NMDS_{mode}_S={nmds$stress}.pdf'), 
  ...
  write.csv(perMANOVA, glue('./18s_R_perMANOVA_{mode}.csv'))
  ...
  ggsave(glue('./18s_R_BDISPER_{mode}.pdf'), 
         width=6000, height=5400,
...
for(mode in trophic_list){
  ...
  p <- ggplot(data.scores, aes(x=NMDS1, y=NMDS2, color=block)) +
    ...
    guides(colour=guide_legend(title='Block'),
           fill=guide_legend(title='Block'))
  ggsave(glue('./18s_NR_NMDS_{mode}_S={nmds$stress}.pdf'), 
         width=6000, height=5400,
         units='px', plot = p, dpi = 500)
  
  perMANOVA <- adonis2(hell~block+PC1+heat_load+grass_volume+simpson_dom, 
                       data=metadata, permutations=perm.ctrl, method='bray')
  write.csv(perMANOVA, glue('./18s_NR_perMANOVA_{mode}.csv'))
  ...
  ggsave(glue('./18s_NR_BDISPER_{mode}.pdf'), 
         width=6000, height=5400,
         units='px', plot = b, dpi = 500)
