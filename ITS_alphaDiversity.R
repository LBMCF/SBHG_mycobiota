## Alpha diversity metrics code for ITS dataset analysis generated and presented in  the manuscript entitled 
...
blocks <- ddOTUtable %>%
  group_by(block) %>%
  summarise(across(starts_with('SH'), sum)) %>%
  as.data.frame()
...
for(mode in trophic_list){
  ...
  ggsave(glue('./ITS_q_orders_{mode}.pdf'), 
